...
class TestReplayEndpoint:
    def test_replay_exists(self, client, tmp_path):
        client.app.state.session.game_id = "testgame"
        client.app.state.session.data_dir = tmp_path
        rec_dir = tmp_path / "testgame" / "rec"
        rec_dir.mkdir(parents=True)
        (rec_dir / "ref_abc.replay").write_bytes(b"RPLA")

        resp = client.get("/api/references/ref_abc/replay")
        assert resp.status_code == 200
...
class TestReplayByRefId:
    ...
    def test_replay_start_resolves_session_replay_path(self, client, db, tmp_path):
        ...
        assert resp.status_code == 200
        # Must resolve to the .replay path built from run_id + ordinal
        client.app.state.session.start_replay.assert_called_once_with(str(session_replay), speed=2)


class TestListReferencesHasReplay:
    def test_list_references_includes_has_replay(self, client, db, tmp_path):
        ...
        assert len(refs) == 1
        assert refs[0]["has_replay"] is True
    ...
    def test_list_references_has_replay_false_when_missing(self, client, db, tmp_path):
        ...
        refs = resp.json()["references"]
        assert len(refs) == 1
        assert refs[0]["has_replay"] is False

    def test_list_references_has_replay_via_session(self, client, db, tmp_path):
        """Post-multi-session run: has_replay=True when session's .replay file exists."""
        client.app.state.session.data_dir = tmp_path
        # Finalized (draft=0) run with a capture_session whose .replay file is on disk
        db.create_capture_run("ref2", "test_game", "Run 2")
        rec_dir = tmp_path / "test_game" / "rec"
        rec_dir.mkdir(parents=True)
        replay_file = rec_dir / "ref2__sess001.replay"
        replay_file.write_bytes(b"RPLA")
        db.create_capture_session(
