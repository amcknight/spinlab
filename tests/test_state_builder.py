...
def _make_sm(mode: Mode, game_id: str | None = "g1") -> MagicMock:
    """Build a minimal SessionManager mock for StateBuilder.build()."""
    sm = MagicMock()
    sm.mode = mode
    sm.tcp.is_connected = True
    sm.game_id = game_id
    sm.game_name = "Test" if game_id else None
...
class TestIdleBaseCase:
    def test_no_game_returns_bare_state(self, mock_db):
        ...
        sm = _make_sm(Mode.IDLE, game_id=None)
