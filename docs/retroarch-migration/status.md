...
- **`test_replay_fixture.py` and `test_smoke.py`** stay Mesen-bound until Phase E. They exercise paths that need `.spinrec` replay and full dashboard+emulator+DB stacks — neither has an RA-driven equivalent yet. They skip cleanly when their respective Mesen fixtures aren't available.
