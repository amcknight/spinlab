...
- **Practice reload-on-death after first death.** Reproducer (Andrew, 2026-05-07): first koopa hit reloads correctly; second identical hit does not. Diagnostic logging added in `RetroArchOrchestrator._maybe_reload_state_on_death` will log `"practice reload-on-death triggered"` or `"practice reload-on-death skipped: timing not armed"` — next test should reveal whether `is_armed` flipped or whether the Death/LevelExit event itself stopped firing.
