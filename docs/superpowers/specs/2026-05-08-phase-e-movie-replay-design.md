...
The 2026-05-08 RA poke harness landing closed P1.2 — the integration tests that don't require input replay now pass under RA. The remaining backend-specific failures are all in replay territory (`test_replay_fixture.py` skips, `/api/replay/start` returns 501). Before committing to the full input-recording rewrite, Andrew wants validation that movie record/playback is reachable, deterministic, and compatible with the live poller in our specific RA configuration. Three foundational unknowns have never been confirmed (per spike-log.md, movie feasibility was deferred during Phase 2):
...
- **Playback commands:** TBD — Task 4 follow-up probe. Candidates: `PLAY_REPLAY`, `HALT_REPLAY`, or hotkey-sim equivalents.
...
3. **Three new RA-backend integration tests** in `tests/integration/test_movie_smoke.py`: record-toggle smoke, playback determinism, polling-during-playback.
4. **Port `tests/integration/test_replay_fixture.py`** from Mesen+`.spinrec` to RA+`.replay`. Same segment-count and segment-structure assertions.
5. **`tests/fixtures/love_yourself/one_level.replay`** — Andrew records this manually after step 2 lands. Replaces `two_level.spinrec` as the replay fixture input.
6. **Unit tests** for `MovieRecorder` and `MoviePlayer` against a fake NCI client.

### Out of scope (deferred)

- **`.spinrec` → `.replay` converter.** Andrew's existing fixtures are re-recorded from scratch.
- **User-facing movie record toggle** for arbitrary play sessions. Reference-run record only.
...
Each step's tests pass before the next begins. Steps 1 and 4 are real go/no-go gates: if step 1 fails on every candidate control path, movie record/playback is unreachable from our setup and the spec halts (see "Risks and mitigations" for the contingency).

### Step 1 — `test_record_toggle_smoke`

**Goal.** Validate that we can start and stop movie recording from Python via NCI, mid-session, and produce a non-zero `.replay` file. ~½ day.

**File.** `tests/integration/test_movie_smoke.py`. Uses the existing `ra_harness` session fixture from Plan 2.
...
1. NCI `RECORD_REPLAY` (confirmed working on RA 1.22.2).
2. NCI hotkey-sim equivalent for `record_replay`. May be filtered at RA's input layer (Phase 0 found this for `MENU_TOGGLE`).
...
```python
def test_record_toggle_smoke(ra_harness):
    movie_dir = discover_movie_dir(ra_harness.client)   # via GET_CONFIG_PARAM or fs sweep
    baseline_files = set(movie_dir.glob("*.replay*"))

    record_replay(ra_harness.client)   # candidate path
    for _ in range(30):
        ra_harness.client.frame_advance()
    halt_replay(ra_harness.client)

    new_files = set(movie_dir.glob("*.replay*")) - baseline_files
    assert len(new_files) == 1
    assert new_files.pop().stat().st_size > 0
    assert ra_harness.client.get_status().is_responsive   # no deep-pause, no crash
```

**Open during step 1.** Where the `.replay` file lands. RA's default movie dir is configurable (`movie_directory` in cfg). Test reads via NCI `GET_CONFIG_PARAM` if available, else does an mtime sweep of plausible directories. Whichever works gets fixed in `EmulatorConfig.ra_movie_dir`.
...
class MovieRecorder:
    client: NCIClient
    movie_dir: Path                   # where RA writes .replay files (from cfg)
    ...
    def stop(self) -> Path:
        ...
        # 2. Poll movie_dir for the new .replay file (mtime > baseline), 5 retries × 200ms
...
**Wiring into `ReferenceController`.** `__init__` gains an optional `movie_recorder: MovieRecorder | None`. `start()` calls `recorder.start(rec_dir / f"{ref_id}.replay")` when present. `stop()` calls `recorder.stop()`. None-by-default — unit tests don't need a recorder; the dashboard's `RetroArchOrchestrator` constructs one and passes it in.

**Failure mode.** If recording fails (NCI error, file never appears, move fails), log a warning and continue. Reference runs are about state captures; movie recording is supplementary until step 5 wires playback into something user-facing. A failed recording does not abort or invalidate the reference run.
...
After step 2 lands, Andrew runs a 1-level reference on Love Yourself, finishes the level, saves. The resulting `.replay` file is written to `{config.data_dir}/{game_id}/rec/{ref_id}.replay`. Andrew copies it to `tests/fixtures/love_yourself/one_level.replay`. A sibling `tests/fixtures/love_yourself/one_level.json` records:
...
### Step 4 — `test_movie_playback_deterministic` and `test_poller_runs_during_playback`

Both tests live in `test_movie_smoke.py` alongside step 1's test, both use the real fixture from step 3.

**`test_movie_playback_deterministic`.** Validates movie playback produces deterministic memory state under our exact config (runahead=2, secondary-instance=true, cheevos-off):

```python
def test_movie_playback_deterministic(ra_harness, fixture_metadata):
    fixture = Path("tests/fixtures/love_yourself/one_level.replay")
...
**`test_poller_runs_during_playback`.** Validates the live `Poller` can keep up during movie playback:
...
The poller doesn't change. It polls memory regardless of whether RA is in movie playback or live play, and the same `_handle_replay_*` codepath in `session_manager.py` produces the dashboard frame-progress events from the polled state.

### Step 6 — Port `test_replay_fixture.py` to RA + movie replay

Replace `replay_dashboard` fixture's Mesen process with an RA process (likely a new `replay_ra_process` fixture in `conftest.py`). Replace `.spinrec` copy with `.replay` copy. Same assertions about segment counts (4 segments for two_level → adapts to whatever step 3 produces for one_level; metadata file declares the expected count) and segment structure (entrance→checkpoint, checkpoint→goal pairs per level).
...
1. **Power-on only.** Movie always records from cartridge insert. To capture a mid-run reference, recording would need to be running from RA launch and truncated at reference boundaries. Replay always starts from boot.
2. **Savestate-anchored.** Movie record toggles relative to current state; replay loads an associated `.state` and plays inputs from there. This is what the original Phase E spec assumed.
...
What this affects:
- **Model 1:** `MovieRecorder.start()` is a "split point marker," not a fresh record. Reference runs would need to pair `.replay` with the boot-time state, OR we accept that replay always starts from boot (ugly for partial-run replays).
...
Even if movie plays back, three things could break determinism:

- **Runahead.** Secondary instance is enabled (per Phase 0's `run_ahead_secondary_instance = "true"` requirement); interactions between runahead and movie playback are undocumented. The determinism test must run with runahead=2 enabled to catch this.
- **Cheevos hardcore.** Already known to silently no-op state ops (spike-log.md). If it also disables movie recording, step 1 fails. Test asserts cheevos is off and skips with a clear message if not.
...
Movie playback proceeds at full speed by default. If the poller can't keep up, transitions get missed during replay. The polling test in step 4 measures success rate; if <90%, throttle movie playback speed via NCI rather than complicate the poller.

## Risks and mitigations

- **Risk: step 1 fails on every candidate control path.** Movie record/playback is unreachable from our setup. Recovery options, in order: (a) investigate whether a different RA build behaves differently and update; (b) Network RetroPad as input-injection layer (Phase 0 noted this as the fallback) — drops back to building our own movie format on top of NCI input commands, significantly larger; (c) document Phase E as blocked and reconsider whether RA replay is required for "full parity" or whether replay is a feature we drop. Decision deferred to the failure event.
- **Risk: movie playback is non-deterministic.** Step 4's test catches this. If the determinism test flakes, investigation order: (1) confirm cheevos and runahead config; (2) compare against Andrew's smoke-testing observations; (3) consider whether snes9x_libretro version or core options affect determinism. The spec halts at this discovery — replay built on non-deterministic playback is not useful.
- **Risk: poller starvation during playback.** Step 4's test catches this. Mitigation: throttle movie playback speed (NCI `SLOWMOTION_RATIO` or equivalent) until reads succeed at the required rate. Acceptable to slow replay for the trade — replay is not real-time-critical the way live play is.
- **Risk: writing recorded `.replay` files to disk consumes space if playback is broken.** Small. `.replay` files for one level are tens of KB. Accept the dead bytes if they happen.
- **Risk: movie format changes between RA versions.** Out of scope — we ship against RA 1.22.2 and the test suite catches break-on-upgrade. Address if it becomes a real problem.

## Definition of done

- [ ] `tests/integration/test_movie_smoke.py` passes under `backend=retroarch`: three tests covering record-toggle, playback determinism, polling-during-playback.
- [ ] `MovieRecorder` integrated into `ReferenceController`; reference runs produce `<refid>.replay` alongside `<refid>.mss`. Recording failures log a warning and don't abort the run.
...
- [ ] `tests/fixtures/love_yourself/one_level.replay` committed, with sibling `one_level.json` declaring `frame_count`, `expected_segments`, and `determinism_probe`.
- [ ] `tests/integration/test_replay_fixture.py` ported to RA + movie replay, passes 5 consecutive runs with no flakes.
- [ ] Mesen-side `tests/integration/test_replay_fixture.py` deleted (option (a)).
- [ ] `tests/unit/test_movie_recorder.py` and `tests/unit/test_movie_player.py` cover the recorder and player against a fake NCI client (file-shuffle, state machine, error paths).
- [ ] `python -m pytest` runs clean.

### Smoke-testing gates (Andrew → option (b))

After this spec ships, Andrew smoke-tests against real reference runs. The following gates move us from option (a) → option (b) (full parity, including user-facing replay endpoint and reference-run movie-by-default):
...
- **Option (b): full parity.** User-facing `/api/replay/start` accepts arbitrary `.replay` files. Movie recording becomes load-bearing for the practice loop, not just a regression-test fixture.
...
- Original Phase E sketch (frozen historical artifact): [`docs/superpowers/specs/2026-05-06-retroarch-migration-design.md`](2026-05-06-retroarch-migration-design.md) §"Phase E — Replay via BSV" (historical name)
...
- Spike log (movie replay deferred): [`docs/retroarch-migration/spike-log.md`](../../retroarch-migration/spike-log.md)
