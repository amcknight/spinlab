...
def run_fast_tests_with_contexts() -> None:
    ...
    cmd = [
        ...
        # contexts; don't also pass --cov-context=test (they conflict)
        "-q",
    ]
...
def module_coverage(conn: sqlite3.Connection, module: Path) -> tuple[int, int, int]:
    ...
    Uses executable_lines from coverage.analysis2 as the denominator.
    Raw numbits bitmaps include non-statement lines (decorators, docstrings,
    class/def headers) that the tracer fires for; intersecting with
    executable_lines keeps percentages bounded by 100%.
    """
    # Use the full relative path to avoid ambiguity when two modules share the
    # same basename (e.g. spinlab/practice.py vs routes/practice.py).
    # Coverage stores absolute paths; match via a trailing suffix that includes
    # enough path components to be unique.
    ...
    raw_suite_lines: set[int] = set()
    for (numbits,) in rows:
        raw_suite_lines |= _numbits_to_lines(numbits)

    # Dedicated: only contexts from tests/test_<module>.py
    test_file = dedicated_test_file(module)
    raw_dedicated_lines: set[int] = set()
    if test_file is not None:
        # dynamic_context = "test_function" produces dot-separated context IDs:
        #   test_session_manager.TestClass.method_name
        # Match all contexts from tests/test_<module>.py with LIKE 'test_stem.%'
        ...
        for (numbits,) in rows:
            raw_dedicated_lines |= _numbits_to_lines(numbits)

    # Intersect with executable lines so percentages stay ≤ 100%.
    executable = _executable_lines(module)
    total = len(executable)
    suite = len(raw_suite_lines & executable)
    dedicated = len(raw_dedicated_lines & executable)

    return (total, suite, dedicated)
...
def _executable_lines(module: Path) -> set[int]:
    """Return the set of executable line numbers in a module via coverage.py."""
